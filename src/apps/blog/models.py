...
class BlogConfig(SingletonModel):
    ...
    def get_absolute_url(self):
        return resolve_url('support:blog')
...
class Author(models.Model):
    """ Автор """
    first_name = models.CharField(_('title'), max_length=255)
    last_name = AutoSlugField(_('slug'), populate_from='title', unique=True)
    photo = StdImageField(_('photo'),
        blank=True,
        storage=MediaStorage('blog/photos'),
...
class Tag(models.Model):
    ...
    def get_absolute_url(self):
        return resolve_url('support:blog_tag', tag_slug=self.slug)
...
class BlogPost(models.Model):
    ...
    author = models.ForeignKey(to=Author, verbose_name=_('author'), blank=True, null=True)
    ...
    def get_absolute_url(self):
        return resolve_url('support:blog_detail', slug=self.slug)
