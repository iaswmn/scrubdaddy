...
from .models import BlogConfig, BlogPost, Tag, Author
...
class AuthorAdmin(ModelAdminMixin, SortableModelAdmin):
    """ Автор """
    fieldsets = (
        (None, {
...
class BlogPostAdmin(SeoModelAdminMixin, AutoPostMixin, admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'classes': ('suit-tab', 'suit-tab-general'),
            'fields': ('preview', 'header', 'slug', 'tags', 'status', 'date', 'author'),
        }),
        (_('Content'), {
            'classes': ('suit-tab', 'suit-tab-general'),
            'fields': ('note', 'text'),
